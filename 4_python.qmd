...
For this tutorial, we'll use Python code chunks in Quarto, which can execute Python code just like it executes R code.
...
A Python environment is an isolated directory that contains: - A specific Python version - Installed packages and their specific versions - Dependencies for those packages
...
print(fruits[-1])     # Last element (negative indexing from end)
...
::: {.callout-tip title="Compared to R"}
Most operators are the same. Key differences: - `/` always returns float in Python 3 (R returns integer if both are integers) - `//` for integer division (R: `%/%`) - `**` for exponentiation (R traditionally uses `^`, though `**` now works)
::: 
...
while count < 5:
    ...
    count += 1  # Increment (Python doesn't have count++)
