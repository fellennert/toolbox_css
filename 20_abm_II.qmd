...
2. **Calibration Techniques (Chapter 3)**  
...
3. **Hands-On Exercises (Chapter 4)**  
...
- **Software Requirements**: We will use R (version 4.0 or later), with packages including `tidyverse` and preferably with `foreach` and `doParallel`.
- **Data**: Synthetic data (`historic_data.RData`) will be provided.  
- **Code**: Functions for simulation and calibration reside in separate R scripts (`functions_simulation.R`, `functions_calibration.R`). We will call them throughout.
...
3. Visualization of the grid
4. Movement and contact logic
5. The simulation loop
6. Reporting and summarizing results
...
1. Draw a random distance \\( d \\) from a specified distribution (e.g., **geometric**, **Weibull**, etc.) using the **`agent_distance_function`**.  
...
Now the world is a data frame, for each agent a row with its x and y coordinates, the cell id, and its innovation-status (1 = innovator, 0 = not).
...
4. Filtering out innovators who fail a random threshold (they won’t successfully convince anyone).
5. Updating the targeted agents’ status if they have been convinced to adopt the innovation.
...
1. Static Plots: Snapshot views of the world at specific time steps.
2. Animations: Dynamic visualizations showing the evolution of the innovation over time.
...
1. Reducing Data: Generating Comparable Reports
2. Properties of a Good Fit Function
3. Defining the Fit Function
4. Example: Calibrating to Synthetic Historic Data
...
### Understanding the Report
    ...
    s_1 to s_7: The number of cells that fall into specific thresholds of local innovation shares:
        ...
        s_2 to s_7: Cells with 1/6 up to 6/6 (100%) innovators.
...
In the next step, we first compare the historic data with our prior run. For this, we measure the fit at each time-step. Perfect overall fit -- i.e., if our model perfectly replicates the historic data, would be 0.
```{r eval=TRUE}
source("files/functions_simulation.R")
