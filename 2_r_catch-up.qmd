...
outcome <- vector("list", length = 1000)
for (i in 1:1000) {
  outcome[[i]] <- play_roulette(bet = 1, number = 20)
}

bind_rows(outcome) |> 
  summarize(total_return = sum(your_return), profit = total_return - 1000)
```
...
play_roulette_restricted(number = 37)
```
...
play_roulette_basic <- function(bet = 1, number) {
  if (number > 36) stop("You can only bet on numbers between 0 and 36, dumbass.")
  draw <- sample(0:36, 1)
  if (number == draw) {
