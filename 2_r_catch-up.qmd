...
Note that you could also do a presentation (with the `beamer` package), a `shiny` app, or use templates. We will focus on simple Quarto documents. Here, you can type in a title, the name(s) of the author(s), and choose the default output format. For now you have to choose one, but later you can switch to one of the others whenever you want to.
...
vembedr::embed_youtube("SaDPmaezwiw")
```
...
As `|>` is a bit tedious to type, there exist shortcuts: `shift-ctrl-m`.
...
```{r include=FALSE}
election_data <- bind_rows(
  read_csv("https://www.dropbox.com/s/14k6bkmaq6l47p2/midwest.csv?dl=1"), 
  read_csv("https://www.dropbox.com/s/t3057jf9evt6vjz/northeast.csv?dl=1") |> 
