...
turnout_votes <- read_delim("files/kerg.csv", skip = 2, delim = ";", show_col_types = FALSE) |> 
  ...
  left_join(read_delim("files/kerg.csv", skip = 2, delim = ";") |> 
              filter(`gehört zu` == 99) |> 
              select(1:2),
            by = c("nr_filled" = "Nr")) |> 
  relocate(Gebiet) |> 
